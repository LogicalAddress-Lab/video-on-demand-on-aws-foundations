 ...
 */
 ...
 exports.handler = async (event,context) => {
     ...
     try {
         ...
         let settingsFile;
         let fileExt = srcVideo.split('.').pop();
         if(JOB_SETTINGS_AUDIO && (srcVideo.indexOf('/tracks/') >= 0 || ['mp3', 'wav', 'ogg', 'wma', 'aac'].indexOf(fileExt) >= 0)){
             settingsFile = `${srcVideo.split("/")[0]}/${JOB_SETTINGS_AUDIO}`;
         }else{
             // VIDEO is default
             settingsFile = `${srcVideo.split("/")[0]}/${JOB_SETTINGS}`;
         }
